...
<script>
  $('#periodbill').on("change", function() {
    ...
    $('#add').click(function(){
        var sendPeriod = $("#periodbill").find(':selected').val();
        var tarif = <?= $dataRates == null ? '' : $dataRates->id; ?>;

        $.ajax({
            type : 'POST',
            url  : '<?php echo base_url('GL/servicePeriod'); ?>',
            data : {period : sendPeriod, tarif: tarif},
        ...
        $.ajax({
            type : 'POST',
            url  : '<?php echo base_url('AR/servicePeriod'); ?>',
            data : {period : sendPeriod, tarif: tarif},
            success : function(data){
              console.log(sendPeriod);
            }
